...
const Blog = () => {
    ...
    const filteredPosts = useMemo(() => sortedPosts.filter(post => post.title.toLowerCase().includes(searchTerm.toLowerCase())), [sortedPosts, searchTerm]);
    ...
    return (
        <div className='relative flex flex-col'>
            <div className="flex flex-col items-center justify-center bg-[#0b8b68] py-6 md:py-16 px-6 gap-4">
            ...
            <div className='app__flex py-1 w-full bg-[#f4f4f4] border-y items-center'>
                <input 
                    className="border text-black text-sm py-1 md:py-2 px-2 rounded-l-sm font-bold outline-none focus:border-[#0b8b68] transition-all duration-300" 
                    type="search" 
                    name="search" 
                    value={searchTerm} 
            ...
            <div className='app__flex gap-2 rounded-sm py-1 w-full bg-[#f4f4f4] text-white font-bold border-y'>
                {currentPage < Math.ceil(filteredPosts.length / postsPerPage) - 1 && (
                    <button 
                        className='flex items-center gap-4 bg-[#0b8b68] hover:bg-[#034b38] text-xs py-2 px-4 ease-in duration-300' 
